(() => {
  ...
  const descriptions = {
    ...
    arena: 'Change les dimensions utiles : grille, labyrinthe, briques, voies et distance selon le jeu arcade.',
    bonus: 'Active la variante bonus propre au jeu arcade sélectionné.',
    hintLimit: 'Limite le nombre de déductions offertes ; chaque indice réduit le score final.',
    abandon: 'Termine la partie sans victoire tout en laissant le plateau visible.',
    ...
    format: 'Choisit les dimensions du plateau ou de la zone de jeu.',
    target: 'Détermine la valeur, la distance ou le score à atteindre pour gagner.',
    pairs: 'Choisit le nombre de paires différentes à mémoriser.',
    theme: 'Change la famille de symboles utilisée sans modifier les règles.',
    stemMode: 'Choisit les sources acoustiques que l’outil local devra extraire.',
    startLength: 'Fixe le nombre de sons présents dans la première séquence.',
    gameMode: 'Choisit une campagne finie, une survie sans limite ou une succession de boss.',
    fireMode: 'Active le tir automatique ou exige de maintenir la commande de tir.',
    soundEnabled: 'Active les sons synthétisés localement pour les tirs, impacts, bonus et explosions.',
  ...
  const selectedDescriptions = {
    difficulty: { easy: 'Plus d’indices ou des adversaires plus lents.', normal: 'Équilibre standard entre aide et complexité.', hard: 'Moins d’indices et davantage de recherche.', expert: 'Très peu d’aide et contraintes maximales.', extreme: 'Réactivité ou complexité maximale.' },
    mode: { mono: 'Grille en noir et blanc.', color: 'Chaque groupe possède une couleur distincte.' },
    arena: { compact: 'Terrain court et dense.', standard: 'Dimensions équilibrées.', large: 'Terrain plus grand et partie plus longue.' },
    bonus: { off: 'Conserve uniquement les règles classiques.', on: 'Ajoute la règle bonus propre au jeu sélectionné.' },
    hintLimit: { unlimited: 'Les indices restent disponibles sans limite.', '3': 'Trois indices maximum.', '1': 'Un seul indice pour toute la partie.', '0': 'Aucun indice autorisé.' }
  };

  const style = document.createElement('style');
  style.textContent = `.game-options-menu{position:relative;z-index:20}.game-options-menu>summary{list-style:none;cursor:pointer;padding:8px 12px;border:1px solid #8da0ba;border-radius:9px;background:var(--options-bg,#fff);color:inherit;font-weight:800}.game-options-menu>summary::-webkit-details-marker{display:none}.game-options-menu[open]>summary{outline:3px solid #60a5fa55}.game-options-content{position:fixed;z-index:2147483640;width:min(440px,calc(100vw - 24px));max-height:calc(100dvh - 24px);overflow:auto;overscroll-behavior:contain;padding:12px;border:1px solid #8da0ba;border-radius:12px;background:var(--options-panel,#fff);box-shadow:0 14px 35px #0008;color:inherit}.game-options-section{display:grid;grid-template-columns:repeat(auto-fit,minmax(165px,1fr));gap:8px;padding:8px 0}.game-options-section+ .game-options-section{border-top:1px solid #94a3b866}.game-options-section>strong{grid-column:1/-1;color:inherit}.game-options-section label{display:flex;gap:7px;align-items:center;justify-content:space-between;min-width:0;padding:6px 7px;border-radius:7px;background:#94a3b818}.game-options-section label select,.game-options-section label input:not([type=checkbox]){max-width:100%;min-width:0}.option-help-target{text-decoration-decoration-style:dotted}.rule-help-target{cursor:pointer;transition:background .15s,outline-color .15s}.rule-help-target:hover,.rule-help-target:focus{background:#fef3c7;outline:2px solid #f59e0b}.rule-example-dialog{width:min(620px,calc(100vw - 28px));max-height:calc(100dvh - 28px);overflow:auto;padding:18px;border:1px solid #64748b;border-radius:14px;background:#fff;color:#17243a;box-shadow:0 18px 50px #0009}.rule-example-dialog::backdrop{background:#0f172a99;backdrop-filter:blur(3px)}.rule-example-stage{display:grid;place-items:center;min-height:190px;margin:14px 0;padding:18px;border-radius:12px;background:#e7eef8}.rule-example-grid{display:grid;grid-template-columns:repeat(var(--example-columns,3),52px);gap:3px;padding:5px;background:#26394f}.rule-example-cell{width:52px;height:52px;display:grid;place-items:center;background:#fff;color:#17243a;font-size:22px;font-weight:900;position:relative}.rule-example-cell.highlight{background:#bbf7d0;outline:4px solid #16a34a;z-index:2}.rule-example-cell.bad{background:#fecaca;outline:4px solid #dc2626}.rule-example-sign{display:grid;place-items:center;color:#b45309;font-size:24px;font-weight:900}.rule-example-actions{display:flex;justify-content:flex-end;gap:8px}.option-help-tooltip{position:fixed;z-index:2147483647;max-width:min(330px,calc(100vw - 24px));padding:8px 10px;border:1px solid #334155;border-radius:8px;background:#17243a;color:#fff;font:500 13px/1.35 Arial,sans-serif;box-shadow:0 8px 24px #0006;pointer-events:none;opacity:0;transform:translateY(4px);transition:opacity .12s,transform .12s}.option-help-tooltip.visible{opacity:1;transform:none}@media(max-width:520px){.game-options-section{grid-template-columns:1fr}.game-options-section label{align-items:flex-start;flex-wrap:wrap}.game-options-section label select,.game-options-section label input:not([type=checkbox]){width:100%}}@media(prefers-color-scheme:dark){.game-options-menu{--options-bg:#263447;--options-panel:#172235}.game-options-content,.rule-example-dialog{border-color:#60718a;background:#172235;color:#eef4ff}.rule-example-stage{background:#101827}.game-options-section label{background:#94a3b824}.rule-help-target:hover,.rule-help-target:focus{background:#5b4514;outline-color:#fbbf24}.option-help-tooltip{border-color:#94a3b8;background:#eef4ff;color:#17243a}}`;
  ...
  const ruleDialog = document.createElement('dialog');
  ruleDialog.className = 'rule-example-dialog';
  ruleDialog.innerHTML = '<h2 data-rule-title>Exemple de règle</h2><p data-rule-text></p><div class="rule-example-stage" data-rule-stage></div><p data-rule-explanation></p><div class="rule-example-actions"><button type="button" data-rule-close>Fermer</button></div>';
  ruleDialog.querySelector('[data-rule-close]').onclick = () => ruleDialog.close();
  document.body.append(ruleDialog);
  ...
  const fallbackDescription = element => {
    ...
    if (element.matches('.rules li,.legend li')) return `Règle : ${label}. Exemple : les éléments concernés sont mis en évidence pendant la lecture de cette règle.`;
  ...
  const descriptionFor = element => {
    ...
    const selectedDetail = select && (select.selectedOptions?.[0]?.dataset.help || selectedDescriptions[select.id]?.[select.value]);
    return selectedText ? `${base} Sélection actuelle : « ${selectedText} ».${selectedDetail ? ` ${selectedDetail}` : ''}` : base;
  };
  const genericRuleExample = element => ({
    title: 'Exemple visuel',
    text: element.textContent.trim(),
    explanation: 'Les cases vertes satisfont la règle ; la case rouge montre une configuration à éviter.',
    html: '<div class="rule-example-grid"><div class="rule-example-cell highlight">1</div><div class="rule-example-cell">2</div><div class="rule-example-cell">3</div><div class="rule-example-cell">2</div><div class="rule-example-cell bad">2</div><div class="rule-example-cell">1</div></div>'
  });
  const showRuleExample = element => {
    const provider = window.GameRuleExamples;
    const example = typeof provider === 'function' ? provider(element) : provider?.[element.dataset.ruleKey] || genericRuleExample(element);
    const resolved = example || genericRuleExample(element);
    ruleDialog.querySelector('[data-rule-title]').textContent = resolved.title || 'Exemple de règle';
    ruleDialog.querySelector('[data-rule-text]').textContent = resolved.text || element.textContent.trim();
    ruleDialog.querySelector('[data-rule-stage]').innerHTML = resolved.html || genericRuleExample(element).html;
    ruleDialog.querySelector('[data-rule-explanation]').textContent = resolved.explanation || '';
    if (typeof ruleDialog.showModal === 'function') ruleDialog.showModal(); else ruleDialog.setAttribute('open', '');
  };
  ...
  const removeNativeTitles = root => {
    if (root instanceof HTMLElement) captureNativeTitle(root);
    root.querySelectorAll?.('[title]').forEach(captureNativeTitle);
  };
  ...
  const suitableTarget = element => {
    ...
    return element.matches('select,input,button[id],label:has(select),label:has(input),.rules li,.legend li');
  };
  const enhance = root => {
    ...
    elements.filter(suitableTarget).forEach(element => {
      ...
      if (element.matches('.rules li,.legend li')) {
        element.classList.add('rule-help-target');
        element.tabIndex = 0;
        element.setAttribute('role', 'button');
        element.addEventListener('click', () => showRuleExample(element));
        element.addEventListener('keydown', event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); showRuleExample(element); } });
      }
  ...
  const setupOptionsMenu = (menu, content) => {
    if (menu.dataset.optionsMenuReady) return;
    menu.dataset.optionsMenuReady = 'true';
    menu.classList.add('game-options-menu');
    menu.open = false;
  ...
  const normalizeExistingMenus = () => {
    document.querySelectorAll('details.option-groups,details.arcade-options').forEach(menu => {
      if (menu.classList.contains('game-options-menu')) return;
      let summary = menu.querySelector(':scope > summary');
      if (!summary) {
        summary = document.createElement('summary');
        menu.prepend(summary);
      }
      summary.textContent = 'Options';
      const content = document.createElement('div');
      content.className = 'game-options-content';
      [...menu.children].filter(child => child !== summary).forEach(child => content.append(child));
      menu.append(content);
      setupOptionsMenu(menu, content);
    });
  };
  const groupContainerOptions = container => {
    if (container.dataset.optionsGrouped || container.closest('.option-groups,.game-options-menu')) return;
    const labels = [...container.children].filter(child => child.matches?.('label') && !child.hasAttribute('data-options-exclude') && child.querySelector('select,input:not([type="file"])'));
  ...
  const initialize = () => {
    ...
    normalizeExistingMenus();
    enhance(document);
    document.querySelectorAll('.toolbar,.bar').forEach(groupContainerOptions);
    new MutationObserver(mutations => mutations.forEach(mutation => {
      if (mutation.type === 'attributes') {
        const target = mutation.target;
        captureNativeTitle(target);
        return;
      }
