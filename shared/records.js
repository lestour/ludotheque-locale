if (!window.GameRecords) {
  if (!document.querySelector('script[data-options-help]')) {
    ...
    script.src = source ? new URL('options-help.js?v=3', source).href : '../../shared/options-help.js?v=3';
  ...
  (() => {
    ...
    function reset() { startedAt = performance.now(); finished = false; updateStatistics({ plays: 1 }); window.setTimeout(render); }
    function exportData() {
    ...
    style.textContent = '.game-record-badge{position:fixed;z-index:9000;right:14px;bottom:max(14px,env(safe-area-inset-bottom));padding:8px 11px;border:1px solid #64748b;border-radius:999px;background:#fff;color:#17243a;box-shadow:0 5px 18px #0003;font:700 12px Arial,sans-serif}.game-record-badge small{display:block;font-size:9px;font-weight:400}.game-record-dialog,.game-result-dialog{max-width:min(680px,calc(100vw - 28px));max-height:calc(100vh - 28px);overflow:auto;padding:18px;border:1px solid #64748b;border-radius:14px;background:#fff;color:#17243a;box-shadow:0 16px 45px #0008}.game-result-dialog{width:min(430px,calc(100vw - 28px));text-align:center}.game-record-dialog::backdrop,.game-result-dialog::backdrop{background:#0f172a99;backdrop-filter:blur(3px)}.game-record-dialog li{margin:7px 0;overflow-wrap:anywhere}.record-actions{display:flex;justify-content:flex-end;gap:8px;flex-wrap:wrap}.game-result-dialog .record-actions{justify-content:center}.game-result-dialog [data-replay]{background:#0f766e;color:#fff}@media(prefers-color-scheme:dark){.game-record-badge,.game-record-dialog,.game-result-dialog{background:#1e293b;color:#f8fafc}}';
