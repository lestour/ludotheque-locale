...
def validate_ghost_snapshot(value):
    if not isinstance(value, dict) or len(value) > 12:
        raise ValueError("Position fantôme invalide.")
    result = {}
    for key in ("x", "y", "vx", "vy", "angle", "progress"):
        if key not in value:
            continue
        number = float(value[key])
        if not math.isfinite(number) or abs(number) > 10_000_000:
            raise ValueError("Coordonnée fantôme invalide.")
        result[key] = max(0.0, min(1.0, number)) if key == "progress" else number
    if "state" in value:
        state = str(value["state"])
        if not re.fullmatch(r"[A-Za-z0-9_-]{1,24}", state):
            raise ValueError("État fantôme invalide.")
        result["state"] = state
    if not any(key in result for key in ("x", "y", "progress")):
        raise ValueError("Le fantôme ne contient aucune position.")
    return result
    ...
    for index, seat in enumerate(seats):
        ...
        if player:
            result.append({
                ...
                "team": seat.get("team", player.get("team")),
            })
        else:
            result.append({"index": index, "kind": "bot", "playerId": None, "name": seat.get("name", f"Bot {index + 1}"), "controller": "bot", "label": seat.get("name", f"Bot {index + 1}"), "team": seat.get("team")})
    return result
...
class LanRooms:
    ...
    def disk_state(self):
        ...
        for room in self.rooms.values():
            stored = dict(room)
            stored["events"] = [event for event in room.get("events", []) if event.get("type") != "ghost"][-MAX_EVENTS:]
            stored["players"] = [{key: value for key, value in player.items() if key != "lastGhostAt"} for player in room.get("players", [])]
    ...
    def emit(self, room, event_type, payload=None, player_id=None, persist=True):
        room["sequence"] += 1
        event = {
    ...
    def join(self, code, name, game=None):
        ...
        with self.lock:
            ...
            team_count = max(2, min(4, int(room.get("options", {}).get("teamCount", 2) or 2)))
            player["team"] = len(room["players"]) % team_count
            room["players"].append(player)
            self.emit(room, "player-joined", {"player": public_player(player)}, player["id"])
            return self.serialize(room, player["id"]), token

    def random_join(self, name, game, seat_count=2, options=None):
        with self.lock:
            requested_format = (options or {}).get("multiplayerFormat")
            requested_teams = int((options or {}).get("teamCount", 2) or 2)
            candidates = [
                ...
                and (not requested_format or room.get("options", {}).get("multiplayerFormat") == requested_format)
                and (requested_format != "teams" or int(room.get("options", {}).get("teamCount", 2) or 2) == requested_teams)
            ]
            if not candidates:
                return self.create(name, game, "public", options or {}, seat_count)
    ...
    def command(self, room, player, command, payload):
        with self.lock:
            if command == "set-options":
                ...
                team_count = max(2, min(4, int(room["options"].get("teamCount", 2) or 2)))
                for index, member in enumerate(room["players"]):
                    member["team"] = int(member.get("team") if member.get("team") is not None else index) % team_count
                    member["ready"] = False
                ...
                if room["phase"] != "lobby" or room.get("options", {}).get("multiplayerFormat") != "teams":
                    raise ValueError("Le choix d’équipe n’est disponible que dans un salon par équipes.")
                team_count = max(2, min(4, int(room.get("options", {}).get("teamCount", 2) or 2)))
                team = int(payload.get("team", -1))
                if not 0 <= team < team_count:
                    raise ValueError("Équipe invalide.")
                player["team"] = team
                player["ready"] = False
                self.emit(room, "team", {"team": team}, player["id"])
            elif command == "start":
                ...
                team_count = max(2, min(4, int(room.get("options", {}).get("teamCount", 2) or 2)))
                room["seats"] = [{"playerId": member["id"], "team": int(member.get("team") if member.get("team") is not None else index % team_count) % team_count} for index, member in enumerate(room["players"])]
                room["seats"].extend({"botId": secrets.token_urlsafe(6), "name": f"Bot {index + 1}", "team": index % team_count} for index in range(len(room["players"]), room["seatCount"]))
                room["phase"] = "playing"
                ...
                if room["phase"] != "playing" or room["paused"]:
                    raise ValueError("La partie n’accepte pas de position fantôme actuellement.")
                now = time.monotonic()
                if now - float(player.get("lastGhostAt", 0)) < 0.12:
                    return self.serialize(room, player["id"])
                player["lastGhostAt"] = now
                snapshot = validate_ghost_snapshot(payload.get("snapshot"))
                self.emit(room, "ghost", {"snapshot": snapshot}, player["id"], persist=False)
...
def validate_options(value):
    ...
    for key, option in value.items():
        if not re.fullmatch(r"[A-Za-z][A-Za-z0-9_-]{0,63}", str(key)):
            continue
        if key == "multiplayerFormat" and option not in {"standard", "coop", "race", "versus", "coopScore", "teams"}:
            continue
        if key == "teamCount":
            option = max(2, min(4, int(option or 2)))
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == "/api/rooms/random":
                room, token = self.server.rooms.random_join(validate_name(payload.get("name")), validate_game(payload.get("game")), validate_seat_count(payload.get("seatCount", 2)), validate_options(payload.get("options", {})))
                self.json_response(HTTPStatus.OK, {"room": room, "token": token})
                return
