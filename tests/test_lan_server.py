...
class LanRoomsTests(unittest.TestCase):
    ...
    def test_public_matchmaking_and_permissions(self):
        first, token = self.rooms.random_join("Alice", "games/board/go.html", options={"multiplayerFormat": "standard", "teamCount": 2})
        second, _ = self.rooms.random_join("Bob", "games/board/go.html")
        self.assertEqual(first["code"], second["code"])
        self.assertEqual(second["options"]["multiplayerFormat"], "standard")
    ...
    def test_public_matchmaking_keeps_multiplayer_formats_separate(self):
        coop, _ = self.rooms.random_join("Alice", "games/grid/sudoku.html", options={"multiplayerFormat": "coop", "teamCount": 2})
        race, _ = self.rooms.random_join("Bob", "games/grid/sudoku.html", options={"multiplayerFormat": "race", "teamCount": 2})
        coop_guest, _ = self.rooms.random_join("Claire", "games/grid/sudoku.html", options={"multiplayerFormat": "coop", "teamCount": 2})
        self.assertNotEqual(coop["code"], race["code"])
        self.assertEqual(coop["code"], coop_guest["code"])

    def test_players_can_choose_teams_before_start(self):
        first, first_token = self.rooms.create("Alice", "games/grid/sudoku.html", "private", {"multiplayerFormat": "teams", "teamCount": 3}, 4)
        ...
        self.rooms.command(room, bob, "set-team", {"team": 2})
        self.assertEqual(bob["team"], 2)
        self.assertFalse(bob["ready"])
        ...
        self.assertEqual(started["seats"][0]["team"], 0)
        self.assertEqual(started["seats"][1]["team"], 2)
        self.assertEqual(len(started["seats"]), 4)

    def test_race_ghost_positions_are_validated_and_ephemeral(self):
        first, first_token = self.rooms.create("Alice", "games/arcade/pocket-platformer.html", "private", {"multiplayerFormat": "versus"}, 2)
        _, second_token = self.rooms.join(first["code"], "Bob", "games/arcade/pocket-platformer.html")
        room, alice = self.rooms.authenticate(first["code"], first_token)
        _, bob = self.rooms.authenticate(first["code"], second_token)
        self.rooms.command(room, alice, "ready", {"ready": True})
        self.rooms.command(room, bob, "ready", {"ready": True})
        self.rooms.command(room, alice, "start", {})
        state = self.rooms.command(room, alice, "ghost", {"snapshot": {"x": 125.5, "y": 300, "progress": 1.8, "state": "air"}})
        ghost = state["events"][-1]
        self.assertEqual(ghost["type"], "ghost")
        self.assertEqual(ghost["payload"]["snapshot"]["progress"], 1.0)
        alice["lastGhostAt"] = 0
        with self.assertRaises(ValueError):
            self.rooms.command(room, alice, "ghost", {"snapshot": {"x": float("inf")}})
