...
function render() {
  ...
  if (complete && !lanFinished) {
    lanFinished = true;
    const score = Math.max(0, 100 - usedHints * 10);
    window.GameRecords?.finish({ score, scoreLabel: `${score} points`, won: true, raceWinner: true, hints: usedHints });
  }
}
...
generateButton.addEventListener('click', () => { size = Number(sizeSelect.value); solverMessage = ''; usedHints = 0; lanFinished = false; clearTrace(); if (importedImage && isColorMode && colorPlayer.every(value => value === null || value === -1)) quantizeImage(importedImage); else if (isColorMode) createColorPuzzle(); else createSolution(); render(); });
resetButton.addEventListener('click', () => { solverMessage = ''; usedHints = 0; lanFinished = false; if (isColorMode) colorPlayer = Array(size * size).fill(null); else player = Array(size * size).fill(0); clearTrace(); render(); });
solveStepButton.addEventListener('click', () => {
  const limit = hintLimitSelect.value === 'unlimited' ? Infinity : Number(hintLimitSelect.value);
  if (usedHints >= limit) { solverMessage = 'La limite d’indices de cette partie est atteinte.'; if (isColorMode) renderColor(); else render(); return; }
  usedHints += 1;
  const result = isColorMode ? buildColorTrace(colorPlayer) : buildLogicalTrace(player);
  traceIsColor = isColorMode;
...
solveAllButton.addEventListener('click', () => {
  if (window.LanMultiplayer?.active || new URLSearchParams(location.search).has('lan')) { solverMessage = 'La résolution complète est désactivée pendant une course multijoueur.'; if (isColorMode) renderColor(); else render(); return; }
...
abandonButton.addEventListener('click', () => {
  if (lanFinished) return;
  lanFinished = true;
  solverMessage = 'Partie abandonnée. La grille reste consultable.';
  window.GameRecords?.finish({ score: 0, scoreLabel: 'Abandon', won: false, raceWinner: false, abandoned: true });
