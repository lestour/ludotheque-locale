...
const settings = {
  easy:{ ghostSpeed:.72, thinking:.35 }, normal:{ ghostSpeed:.88, thinking:.58 },
  hard:{ ghostSpeed:1.02, thinking:.78 }, extreme:{ ghostSpeed:1.16, thinking:.95 }
};
...
function createLevel(level=1,seed=`${Date.now()}:${Math.random()}`){
  ...
  return {random,level,width,height,maze,spawn,home,pellets,initialPellets:pellets.size,powers:new Set(powers.map(cell=>cellKey(cell.x,cell.y))),ghosts,player:{x:spawn.x,y:spawn.y,dir:'right',nextDir:'right',mouth:0},score:0,lives:modeSelect.value==='survival'?1:3,time:0,timeLeft:modeSelect.value==='timed'?120:Infinity,frightened:0,ghostChain:0,fruit:null,fruitTimer:0,completed:false,particles:[]};
}

function startGame(){cancelAnimationFrame(frame);game=createLevel(1,new URLSearchParams(location.search).get('seed'));running=true;paused=false;document.getElementById('overlay').hidden=true;statusElement.textContent='Toutes les lumières sont accessibles. Anticipez les esprits.';window.GameRecords?.reset();lastTime=performance.now();updateHud();draw();frame=requestAnimationFrame(loop);}
...
function canTurn(entity,dir){const [dx,dy]=directions[dir];return centerDistance(entity.x)<.2&&centerDistance(entity.y)<.2&&tileOpen(Math.round(entity.x)+dx,Math.round(entity.y)+dy);}
function moveEntity(entity,dir,speed,delta){const [dx,dy]=directions[dir];entity.x+=dx*speed*delta;entity.y+=dy*speed*delta;const horizontal=dx!==0;if(horizontal)entity.y=Math.round(entity.y);else entity.x=Math.round(entity.x);}
...
function update(delta){
  ...
  const playerSpeed=5.1+Math.min(1.2,(game.level-1)*.08);if(canTurn(game.player,game.player.dir))moveEntity(game.player,game.player.dir,playerSpeed,delta);game.player.mouth+=delta*13;collectAtPlayer();
  const ghostSpeed=(4.35+Math.min(1.35,(game.level-1)*.1))*settings[difficultySelect.value].ghostSpeed*(game.frightened>0?.69:1);
  for(const ghost of game.ghosts){if(game.time<ghost.releasedAt)continue;if(centerDistance(ghost.x)<.08&&centerDistance(ghost.y)<.08)ghost.dir=chooseGhostDirection(ghost);if(canTurn(ghost,ghost.dir))moveEntity(ghost,ghost.dir,ghostSpeed,delta);if(Math.hypot(ghost.x-game.player.x,ghost.y-game.player.y)<.55){if(game.frightened>0){game.ghostChain++;const points=200*2**Math.min(3,game.ghostChain-1);game.score+=points;ghost.x=game.home.x;ghost.y=game.home.y;ghost.releasedAt=game.time+2;game.particles.push({x:ghost.x,y:ghost.y,text:`+${points}`,life:1});}else{loseLife();break;}}}
...
function setDirection(dir){if(!game||!directions[dir])return;game.player.nextDir=dir;}
