...
L’import audio multiformat et la séparation locale facultative sont documentés dans `docs/AUDIO_IMPORT.md`.
...
- `games/grid` : Sudoku, Nonogram, Démineur et Puzzles logiques (Futoshiki, Kakuro, Hidato, Hitori, Nurikabe, Akari, Slitherlink, Numberlink).
- `games/arcade` : Serpent, casse-briques, labyrinthe, course express, Pong, shoot’em up, pilotage inertiel et plateforme procédurale, au clavier comme au tactile.
- `games/cards/classic/casino-plis.html` : 21, poker fermé et jeu de plis local.
- `games/board/tiles-routes.html` : atelier original de territoires, mosaïque et routes colorées ; ce module n’est pas une reproduction de Catan.
